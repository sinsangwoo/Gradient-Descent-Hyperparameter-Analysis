...
def experiment_1_learning_rate(X, y):
    ...
    run_and_plot_experiment(
        title="학습률(Learning Rate)에 따른 Loss 변화", X=X, y=y, configs=configs,
...
def experiment_2_batch_size(X, y):
    ...
    run_and_plot_experiment(
        title="배치 크기(Batch Size)에 따른 Loss 변화", X=X, y=y, configs=configs,
...
def experiment_3_optimizer(X, y):
    ...
    run_and_plot_experiment(
        title="옵티마이저(Optimizer)에 따른 Loss 변화", X=X, y=y, configs=configs,
